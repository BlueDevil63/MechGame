...
{
    string modelDir = "Elpida_unity14";
    ...
    public void ChangePart()
    {
      ...
      //  foreach (Mesh mesh in Part)
       ...
       // }
        ...
        {
            GameObject bodyPart = Instantiate(Resources.Load("Parts/legPart") as GameObject, playerStart.transform.position, Quaternion.identity) as GameObject;

            bodyPart.name = bodyPartNames[i];
           bodyPart.GetComponent<MeshFilter>().mesh = bodyPartMeshes[i];
           // bodyPart.GetComponent<SkinnedMeshRenderer>().BakeMesh = bodyPartMesh[i];
